...
public class MarkExamServiceImpl implements MarkExamService {
    ...
    public Msg recommend(Integer userId) {
        ...
        Topic topic = topicList.get(randomIndex);
//        List<String> a = Arrays.asList(topic.getChoice().split("\n"));
//        topic.setTimu(a);
    ...
    public Msg userSubmitProblem(UserTopic userTopic) {
        int count = userTopicDao.insert(userTopic);
        return ResultUtil.success(userTopic);
    }

    @Override
    public Msg getHistoryProblem(Integer userId) {
        List<UserHistoryTopic> userHistoryTopicList = userTopicDao.selectHistoryProblem(userId);
        return ResultUtil.success(userHistoryTopicList);
    }
...
//            userId = null;
//        }
        List<UserStatistic> userStatisticList = userTopicDao.selectUserStatistic(userId);
        if (CollectionUtils.isEmpty(userStatisticList)) {
            UserStatistic userStatistic = new UserStatistic();
            User user = userDao.selectByPrimaryKey(userId);
            BeanUtils.copyProperties(user, userStatistic);
            userStatistic.setStatistic("0%");
            userStatisticList.add(userStatistic);
            return ResultUtil.success(userStatisticList);
        }
        List<UserStatistic> userStatisticSuccess = userTopicDao.selectUserStatisticSuccess(userId);

        BigDecimal a = new BigDecimal(userStatisticSuccess.size());
        BigDecimal b = new BigDecimal(userStatisticList.size());
        BigDecimal c = (a.multiply(new BigDecimal(100))).divide(b, 2, RoundingMode.HALF_UP);
        userStatisticList.get(0).setStatistic(String.valueOf(c) + "%");
        List<UserStatistic> userStatistics = new ArrayList<>();
        userStatistics.add(userStatisticList.get(0));
        return ResultUtil.success(userStatistics);
    }
