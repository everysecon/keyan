<template>
  <div class="myTest content-bg ">
    <div class="tab-title">
      <div class="tabTitle-item active">答题记录</div>
    </div>
    <div class="tab-item">
      <div class="table">
...
<!--          <el-table-column label="难度" min-width="220">-->
<!--            <template slot-scope="scope">{{scope.row.difficulty}} </template>-->
<!--          </el-table-column>-->
<!--          <el-table-column label="分数" min-width="220">-->
...
<!--          </el-table-column>-->
          ...
          <el-table-column label="用户答案" min-width="220">
            <template slot-scope="scope">{{scope.row.userAnswer}} </template>
          </el-table-column>
          <el-table-column prop="enter_date" label="时间" width="240">
            <template slot-scope="scope">
              <i class="el-icon-time"></i>
              <span style="margin-left: 10px">{{ scope.row.answerTime }}</span>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getTestPaperList(val='') {
      ...
      setTimeout(() => {
        this.$http.get("/getHistoryProblem", { params }).then((res) => {
